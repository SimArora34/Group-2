  ...
  useEffect(() => {
    getCurrentProfile().then((res) => {
      if (res.success && res.data?.full_name) {
        setSenderName(res.data.full_name);
      }
    });
  }, []);
      ...
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.invoiceDoc}>
            ...
            <View style={styles.invoiceTable}>
              <View style={[styles.invoiceTableRow, styles.invoiceTableHead]}>
                <Text style={[styles.invoiceTableCell, styles.invoiceTableHeadText, { flex: 2 }]}>
                  Service
                </Text>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              router.push({
                pathname: "/(wallet)/invoice-send-user",
                params: {
                  clientName,
                  companyName,
                  dueDate,
...
const styles = StyleSheet.create({
  ...
  downloadBtn: { alignSelf: "flex-end" },
  invoiceDoc: {
    backgroundColor: Colors.white,
    borderRadius: 12,
    borderWidth: 1,
  ...
  invoiceNo: { fontWeight: "700", color: Colors.textDark },
  invoiceDate: { color: Colors.textMid },
  invoiceParties: {
    flexDirection: "row",
    justifyContent: "space-between",
    gap: 20,
  ...
  invoicePartyLabel: { fontSize: 12, color: Colors.textLight },
  invoicePartyName: { fontSize: 14, fontWeight: "700", color: Colors.textDark },
  invoicePartyCompany: { fontSize: 13, color: Colors.textMid },
  invoiceDue: {
    backgroundColor: Colors.primaryLight,
    padding: 12,
    borderRadius: 10,
  },
  invoiceDueText: { color: Colors.textDark, fontWeight: "700" },
  invoiceTable: {
    borderWidth: 1,
    borderColor: Colors.border,
    borderRadius: 10,
  ...
  invoiceTableRow: { flexDirection: "row" },
  ...
  invoiceTableHeadText: { fontWeight: "700" },
  invoiceSummary: { gap: 6 },
  invoiceSummaryRow: {
    flexDirection: "row",
    justifyContent: "space-between",
  },
  invoiceSummaryKey: { color: Colors.textMid },
  invoiceSummaryVal: { color: Colors.textDark },
  invoiceSummaryTotal: {
    borderTopWidth: 1,
  ...
  invoiceSummaryTotalKey: { fontWeight: "700", color: Colors.textDark },
  invoiceSummaryTotalVal: { fontWeight: "700", color: Colors.textDark },
  continueBtn: {
    backgroundColor: Colors.primary,
    borderRadius: 10,
    paddingVertical: 16,
