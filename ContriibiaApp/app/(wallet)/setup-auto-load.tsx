...
export default function SetupAutoLoadScreen() {
  ...
  const handleSetup = async () => {
    ...
    setError('');
    setLoading(true);
    // TODO: save auto-load settings via backend
    await new Promise((r) => setTimeout(r, 800));
    setLoading(false);
    setSuccess(true);
  };

  const last4 = '9018'; // placeholder card last 4
      ...
      <SafeAreaView style={styles.container} edges={['bottom']}>
        <View style={styles.successWrap}>
          ...
          <Text style={styles.congratsDesc}>
            ...
            <Text style={styles.bold}>${parseFloat(amount).toFixed(2)}</Text>
            {' '}when your balance drops below{' '}
            <Text style={styles.bold}>${parseFloat(threshold).toFixed(2)}</Text>
            {' '}using **** **** **** {last4}
          </Text>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.fieldLabel}>
            Minimum Threshold{' '}
            <Text style={styles.fieldHint}>(When balance drops to:)</Text>
          </Text>
          <TextInput
            style={styles.input}
            value={threshold}
            onChangeText={(v) => { setThreshold(v); setError(''); }}
            placeholder="$0.00"
            placeholderTextColor={Colors.textPlaceholder}
            keyboardType="decimal-pad"
          />
...
const styles = StyleSheet.create({
  ...
  setupBtn: { backgroundColor: Colors.primary, borderRadius: 10, paddingVertical: 16, alignItems: 'center', marginTop: 8 },
  ...
  congratsDesc: { fontSize: 14, color: Colors.textMid, textAlign: 'center', lineHeight: 22, paddingHorizontal: 8 },
