...
export default function AddNewCardScreen() {
  const { editId, editName, editExpiry, type } = useLocalSearchParams<{
    editId?: string;
    editName?: string;
    editExpiry?: string;
  ...
  const cardType = type === "business" ? "business" : "personal";
  ...
  const handleSave = async () => {
    ...
    if (
      !isEditing &&
      (!address1.trim() || !city.trim() || !province.trim() || !country.trim())
    ) {
    ...
    if (isEditing) {
      ...
      res = await addCard({
        ...
        type: cardType,
        bank_name:
          cardType === "business" ? "Contribiia Business" : "Contribiia Personal",
        ...
        <View style={styles.successWrap}>
          <View style={styles.banner}>
            <Text style={styles.bannerText}>
              {isEditing ? "Card Updated!" : "New Card Added!"}
            </Text>
          </View>
          ...
          <Text style={styles.congratsDesc}>
            Your {cardType} bank card{" "}
            <Text style={styles.bold}>**** **** **** {maskedLast4}</Text>{" "}
            has been successfully {isEditing ? "updated" : "added"}.
          </Text>

          <TouchableOpacity
            style={styles.primaryBtn}
            onPress={() => router.replace("/(wallet)/view-my-cards" as any)}
          >
          ...
          {!isEditing && (
            <>
              ...
              <Text style={styles.sectionLabel}>
                Address Line 1 <Text style={styles.req}>(required)</Text>
              </Text>
              <TextInput
                style={styles.input}
                value={address1}
              ...
              <Text style={styles.sectionLabel}>
                City <Text style={styles.req}>(required)</Text>
              </Text>
              <TextInput
                style={styles.input}
                value={city}
              ...
              <Text style={styles.sectionLabel}>
                Province <Text style={styles.req}>(required)</Text>
              </Text>
              <TextInput
                style={styles.input}
                value={province}
              ...
              <Text style={styles.sectionLabel}>
                Country <Text style={styles.req}>(required)</Text>
              </Text>
              <TextInput
                style={styles.input}
                value={country}
