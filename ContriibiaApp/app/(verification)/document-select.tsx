...
export default function DocumentSelectScreen() {
  ...
  const handleContinue = () => {
    ...
    router.push({
      ...
      params: {
        docType: selectedGovId,
        docLabel:
          mockData.governmentIds.find((d) => d.id === selectedGovId)?.label || '',
      },
    });
  };
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Text style={styles.desc}>
          Choose the document type you would like to provide us. We are legally
          required to ask for two documents.
        </Text>

        <Text style={styles.chooseLabel}>Choose:</Text>
        <Text style={styles.chooseItem}>
          1. <Text style={styles.bold}>One Government ID</Text> that includes your
          name and address
        </Text>
        <Text style={styles.chooseItem}>
          2. <Text style={styles.bold}>One Other Official Document</Text> that
        ...
        {mockData.governmentIds.map((doc) => (
          <TouchableOpacity
            ...
            onPress={() => setSelectedGovId(doc.id)}
          >
            <Text
              style={[
                styles.docLabel,
                selectedGovId === doc.id && styles.docLabelSelected,
              ]}
            >
              {doc.label}
            </Text>
        ...
        {mockData.officialDocuments.map((doc) => (
          <TouchableOpacity
            ...
            style={[
              styles.docRow,
              styles.docRowAccent,
              selectedDoc === doc.id && styles.docRowSelectedAccent,
            ]}
